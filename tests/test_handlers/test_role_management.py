...
async def test_add_admin_role_to_user_by_superadmin(
    ...
    resp = client.patch(
        ...
        headers=create_test_auth_headers_for_user(user_data_who_promoted["user_id"]),
    )
...
async def test_revoke_admin_role_from_user_by_superadmin(
    ...
    for user_data in [user_data_for_revoke, user_data_who_revokes]:
        await create_user_in_database(**user_data)
    resp = client.delete(
        f"/user/admin_privilege?user_id={user_data_for_revoke['user_id']}",
        headers=create_test_auth_headers_for_user(user_data_who_revokes["user_id"]),
    )
    data_from_resp = resp.json()
    assert resp.status_code == 200
    ...
    # Check if correct user privilege was revoked
    assert revoked_user_from_db["user_id"] == user_data_for_revoke["user_id"]
    # Check if user privilege was revoked
...
async def test_revoke_admin_role_from_user_by_wrong_type_of_user(
    ...
    resp = client.delete(
        f"/user/admin_privilege?user_id={user_data_for_revoke['user_id']}",
        headers=create_test_auth_headers_for_user(user_data_who_revoke["user_id"]),
    )
    data_from_resp = resp.json()
