...
async def test_delete_user_not_found(client, create_user_in_database):
    ...
    await create_user_in_database(**user_for_db)
    await create_user_in_database(**user_data)
...
@pytest.mark.parametrize(
    "user_for_deletion, user_who_deletes",
    [
        (
            {
                "user_id": uuid4(),
...
async def test_delete_another_user_error(
    ...
    await create_user_in_database(**user_who_deletes)
    resp = client.delete(
        f"/user/?user_id={user_for_deletion['user_id']}",
        headers=create_test_auth_headers_for_user(user_who_deletes["user_id"]),
    )
...
async def test_reject_delete_superadmin(
    ...
    resp = client.delete(
        f"/user/?user_id={user_for_deletion['user_id']}",
        headers=create_test_auth_headers_for_user(user_for_deletion["user_id"]),
    )
