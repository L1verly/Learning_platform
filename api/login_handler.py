...
async def _get_user_by_email_for_auth(email, db: AsyncSession):
...
async def _get_user_by_id_for_auth(user_id, db: AsyncSession):
    async with db as session:
        async with session.begin():
            user_dal = UserDAL(session)
...
async def authenticate_user(email, password: str, db: AsyncSession) -> Optional[User]:
...
async def get_current_user_from_token(
    ...
    try:
        ...
        raise credentials_exception
    user = await _get_user_by_id_for_auth(user_id=user_id, db=db)
    if user is None:
        raise credentials_exception
    return user
